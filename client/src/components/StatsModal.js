...
const StatsModal = ({ show, onClose, stats, sessionStats, darkMode }) => {
  if (!show) return null;
  ...
  const handleClose = () => {
    try {
      onClose();
      // Google Analytics event for closing stats modal
      window.gtag('event', 'close_stats_modal', {
        'event_category': 'Game',
        'event_label': 'Stats Modal Closed'
      });
    } catch (error) {
  ...
  return (
    ...
    >
      <div className={`p-6 rounded-lg max-w-md w-full ${darkMode ? 'bg-gray-800 text-white' : 'bg-white text-black'}`}>
        ...
        <h3 className="text-xl font-bold mt-6 mb-2">Current Session</h3>
        <div className="grid grid-cols-2 gap-4">
          <div className="text-center">
            <div className="text-3xl font-bold">{sessionStats.wordsPlayed}</div>
            <div className="text-xs">Words Played</div>
          </div>
          <div className="text-center">
            <div className="text-3xl font-bold">{sessionStats.hintsUsed}</div>
            <div className="text-xs">Hints Used</div>
          </div>
        </div>
      </div>
    </motion.div>
